...
- Must expect unit tests under `tests/` mirroring the package layout, with shared fixtures in `tests/conftest.py`; the `doc-issues.json` / toolkit-adapter contract is guaranteed by `doc_issues/models.py` + the generated `doc_issues/schema/doc-issues-v1.0.0-schema.json` (`schema_export.py`), not by static golden fixture files.
