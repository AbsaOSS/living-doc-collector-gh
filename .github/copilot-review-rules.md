...
- Must expect unit tests under `tests/` mirroring the package layout, with shared fixtures in `tests/conftest.py`; the `doc-issues.json` producer contract is guaranteed by `doc_issues/models.py` + the generated `doc_issues/schema/doc-issues-v1.0.0-schema.json` (`schema_export.py`) and the parity check `tests/doc_issues/test_collector.py::test_save_issues_with_audit_data`, not by static golden fixture files. Consumer compatibility (`living-doc-toolkit`'s vendored schema, adapter models, and `CONFIRMED_MIN` / `CONFIRMED_MAX` bounds) is a separate obligation synchronized in that repo, not verified here.
